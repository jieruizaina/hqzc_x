			...
			<div v-for="(item, index) in data.source" :key="index" :class="item.className" :style="item.sty"
				...
				<div class="card_box">
					...
					<div class="bottom">
						...
						<el-button type="primary" color="#00469B" plain @click="see(item)">查看详情</el-button>
					</div>
				</div>
			</div>
		</div>
...
export default {
	...
	setup(props) {
		...
		watch(
			() => props.source,
			(newV, oldV) => {
				if (newV.length && !data.source.length) {
					data.source = computed(props.initial, newV);
				}
			},
			{
				immediate: true,
			...
			return source.map((item, index) => {
				...
				switch (index) {
					case temp3:
						...
						break;
					case temp1:
						transform = `translate(-98%, -50%) scale(0.7)`;
						zIndex = 1;
						break;
					case temp5:
		...
		function see(val){
			router.push({ path: "/details", query: { type:'jrzc', id: val.id, formId: val.formId, columnName:'金融政策'} });
		}
		// 点击哪张图片 哪张图片就到第一个
...
.slide {
	...
	transform: translate(-50%, -50%);
	transition: 0.5s;
	background: linear-gradient(125deg, #D5EEFC 7%, #ECF8FE 43%, #C2E7F8 94%);
